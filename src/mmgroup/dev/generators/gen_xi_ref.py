...
Let :math:`G_{x0}` be the subgroup of the monster group of structure 
:math:`2^{1+24}.\mbox{Co}_1` as described in :cite:`Con85` and 
:cite:`Seysen20`.


The group :math:`G_{x0}` contains a subgroup :math:`N_{x0}` of structure
:math:`2^{1+24}.2^{11}.M_{24}`, where :math:`M_{24}` is the Mathieu 
group acting on 24 elements. :math:`M_{24}`  (and also, to some extent, 
:math:`N_{x0}`) is supported by the c functions in file 
``mat24_functions.c``. 

:math:`G_{x0}` is  generated by :math:`N_{x0}` and an element :math:`\xi`
of order 3, which is defined :cite:`Seysen20`, section 9. :math:`G_{x0}` 
has an extraspecial normal subgroup :math:`Q_{x0}` of structure
:math:`2^{1+24}`. The element :math:`\xi` operates on the 
186884-dimensional representation :math:`\rho` of the monster 
group described in section :ref:`mmrep-label`.

Up to sign, the elements of the extraspecial subgroup :math:`Q_{x0}` 
correspond to the vectors of the Leech lattice modulo 2. Some of 
the basis vectors of the representation  :math:`\rho` correspond
to  elements  :math:`Q_{x0}` of extraspecial subgroup which in turn
correspond to short Leech lattice vectors of norm 4. See
:cite:`Con85`  sect.7 and :cite:`Seysen20` section 6.2. for
backgound.
...
(and not only of the subspace spanned by the basis vectors 
corresponding to short Leech lattice vectors) is given in 
...
Here elements of the Parker loop and elements of the cocode word
...
Using :cite:`Seysen20`, Lemma 9.5, we can easily compute
:math:`\xi^{-e} X \xi^e` for all short elements :math:`X` of
:math:`Q_{x0}` and :math:`e=1,2`. The formulas in that lemma
use functions 
:math:`\gamma : \mathcal{G} \rightarrow \mathcal{G^*}` and
:math:`w_2 : \mathcal{G} \rightarrow \mathcal{G^*}`
defined in ibid., section 3.3.
Here the Golay code :math:`\mathcal{C}` is decomposed into
a direct sum  
:math:`\mathcal{C} = \mathcal{G} \oplus \mathcal{H}` of 
subspaces  :math:`\mathcal{G}`  and  :math:`\mathcal{H}`.
Using that decomposition we may extend  functions 
:math:`\gamma` and :math:`w_2` to :math:`\mathcal{C}`. 
For the Golay  cocode :math:`\mathcal{C^*}` 
there is a similar decomposition
:math:`\mathcal{C^*} = \mathcal{G^*} \oplus \mathcal{H^*}` 
and also a natural isomorphism 
:math:`\mathcal{G^*} \rightarrow  \mathcal{G}`. Using that
isomorphism we may also define  functions 
:math:`\gamma` and :math:`w_2` on :math:`\mathcal{C^*}`. 
