...
:math:`N_{x0}`) is supported by the c function in file 
``mat24_functions.c``. 
...
:math:`X = X_d \cdot X_\delta` of :math:`Q_{x0}` as coded as an integer 
:math:`x`  as follows. 
...
Using Lemma 9.5 in :cite:`Seysen20`, we can easily compute
:math:`\xi^{-e} X \xi^e` for all short elements :math:`X` of
:math:`Q_{x0}` and :math:`e=1,2`. But doing about 200000 such 
computations real time is too inefficient, so we generate a set
of tables containing all these values.


Short vector encoding of the short vectors in :math:`Q_{x0}`
------------------------------------------------------------
  
For each short vector in :math:`Q_{x0}` we compute the number of the box 
and the entry in the box using the information given in (2). Then we store 
the vector in the lowest 19 bits of a 32-bit integer as follows:
