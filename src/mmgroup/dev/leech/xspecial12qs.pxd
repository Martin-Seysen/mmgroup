...
cdef extern from "xspecial12qs.h":
    enum: QBSTATE12_MAXCOLS
    enum: QBSTATE12_MAXROWS
    ctypedef struct qbstate12_type:
        uint32_t maxrows
        uint32_t nrows
        uint32_t ncols
        int32_t  factor
    ...
    int32_t qbstate12_square_matrix_to_xz(qbstate12_type *pqs)
    int32_t qbstate12_xz_to_square_matrix(qbstate12_type *pqs)
    int32_t qbstate12_xz_to_wall(qbstate12_type *pqs)
