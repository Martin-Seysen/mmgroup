...
// Here g is given as an array of five 32-bit integers.
{
    ...
    if (g[I_d] & 1) {  // case g1 is odd
        sign_x ^=  mat24_ploop_cocycle(g[I_x], f);
        f ^= MAT24_THETA_TABLE[(f >> 1) & 0x7ff] & 0x1000;
...
// Here g is given as an array of five 32-bit integers.
{
    ...
    if (exp) {  // case (-1)**parity(g1) * exp = 1 (mod 3)
        a1 = g[I_y];
        a1 ^=  MAT24_THETA_TABLE[(a1 >> 1) & 0x7ff] & 0x1000;
        ...
        b1 = g[I_x];
        b1 ^=  MAT24_THETA_TABLE[(b1 >> 1) & 0x7ff] & 0x1000;
...
// These two arrays may overlap.
{
    ...
    mm_group_n_mul_x(g, g2[I_x] 
        ^ (MAT24_THETA_TABLE[(g2[I_x] >> 1) & 0x7ff] & 0x1000));
    mm_group_n_mul_y(g, g2[I_y] 
        ^ (MAT24_THETA_TABLE[(g2[I_y] >> 1) & 0x7ff] & 0x1000));
...
// not processed.
{
    ...
    switch(tag) {
        ...
        case 2:
           ...
           return 0;
        case 8 + 3:
           op ^= MAT24_THETA_TABLE[(op >> 1) & 0x7ff] & 0x1000;
        case 3:
