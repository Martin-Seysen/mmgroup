...
def compress_table(table, d):
    """yet to be dococumented"""
    assert 0 <= d <= 2
    assert len(table) & 31 == 0
    d, tbl = diff_table(table, d)
    assert tbl is not False
    q = 1 << d
    res, dt = list(zip(*[divmod(x, q) for x in tbl]))
    dt32 = [tuple(dt[i:i+32]) for i in range(0, len(dt), 32)]
    #print(dt32)
    #if d:
...
def analyze_tables():
    ...
    for i in range(5):
        for j in range(2):
            L_SIGN = len(set(SIGN_TAB[i][j]))
            d, res, dt = compress_table(PERM_TAB[i][j], 2)
            assert len(res) << d == len(PERM_TAB[i][j])
            L_DIFF = len(set(dt))
            print("%d, %d: %4d, %4d, d=%d" % (i, j, L_SIGN, L_DIFF, d))
