...
# In order to make an array from module array work with Python 2 and 3,
# we have to declare it e.g. as   array(str('B'), [0]*24).
# Using str() is weard, but I don't know any better way to do it. See
# https://stackoverflow.com/questions/37379607/array-of-unicode-literals?              
...
def vect_to_bit_list(uint32_t v1):
    ...
    cdef uint8_t *pa = a
    cdef uint32_t n, w
    w = mat24_vect_to_bit_list(v1, pa)
    return [a[n] for n in range(w)], [a[n] for n in range(w,24)]
...
def vect_to_gcode(uint32_t v1):
    ...
    assert cn & 0xff000000 == 0, "not a Golay code word"
    return cn


def gcode_to_octad(uint32_t v1):
    cdef uint32_t cn
    cn = mat24_gcode_to_octad(v1)
    assert cn & 0xff000000 == 0, "not a Golay code octad"
    return cn


def vect_to_octad(uint32_t v1):
    cdef uint32_t v
    v  = mat24_vect_to_octad(v1)
    assert v & 0xff000000 == 0, "not a Golay code octad"
    return v
...
def perm_from_heptads(h1, h2):
    ...
    return [p1[i] for i in range(24)] if res == 0 else None
...
def m24num_to_perm(uint32_t u_m24):
    ...
    cdef uint8_t *p_p1 = p1
    cdef int i
    mat24_m24num_to_perm(u_m24, p_p1)
...
#
# These functions are not available in class mat24.Mat24.
###########################################################################
