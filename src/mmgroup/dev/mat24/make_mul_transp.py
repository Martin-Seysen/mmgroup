...
class BitMatrixMulTransp(object):
    ...
    an integer may is at least n * lv bits long. We support n 
    simulataneous multiplications for n being a power of two.
    Then the i-th component of v_i and of a row ov m_i is stored in
    bits  i*lv, ..., i*lv + lv-1  of the corresponding integer.

    """ 

    def __init__(self, lv, lm, n=1):
...
def generate_c_mul_transp(v, m, lv, lm, n=""):
    n = max(1, int(n) if n else 0)
    b = BitMatrixMulTransp(int(lv), int(lm), n)
    b.setnames(str(v), str(m)) 
    c = b.generate()
