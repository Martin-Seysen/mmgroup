"""Auxiliary functions for class  Mat24 


See class Mat24 in the module
"""
...
def _make_de_bruijn_sequence():
    """Create modified DeBruijnSequence for finding the lowest bit
...
class Lsbit24Function(object):
    ...
    for computing that  least significant bit.

    This documentation is intended to explain the cooperation of this
    class with the class make_c_tables.TableGenerator. Classes designed 
    ...
    use this as an exmaple for documenting an application of the code-
    generating class make_c_tables.TableGenerator.


    This class hat the following public member functions

    tables()        Returns a dictionary containing the tables
                    required for genrating the C code
    directives()    Returns a dictionary containing a function for
                    generating code for computing function lsbit24(x).    

    This class is designed for cooperation with an instance of
    class make_c_tables.TableGenerator. A similar class must support
    at least the member functions like tables() and directives().

 
    In order to create the table, put e.g. the lines
    ...
    into the input file. This creates the table MAT24_LSBIT_TABLE for
    computing the lsbit quickly. 
 
    This class provides a function with name MAT24_LSBIT for generating
    C code that compute the lsbit of x. After generating the table,
    the expression {MAT24_LSBIT:x} in the input file generates a C 
    a very efficient expression that evaluates to the lsbit of x. 
    Note that we use standard python formatting syntax, with MAT24_LSBIT
    referring to some internal object associated with function 
    self.generate and 'x' being some formatting parameter. 
 
    Note that python and C name spaces are strictly separated. So the
    table given above has the name  Mat24_lsbit_table in python.
    ...
    may have another name. The code generation process provides a 
    dictionary 'names' that maps python names to C names. Dictionary 
    'names' must be passed to the coder generation function 
    self.generate as the first parameter, so the self.generate() can 
    infer the C name of the table from its python name 
    Mat24_lsbit_table. The peculiar entry 

          "MAT24_LSBIT" : UserFormat(cls.generate, "s", 1)
    
    in self.tables means the following:
