"""Creating table for class Mat24 an using them
...
def make_lex_golay_code_basis():
    """Return sexicographical basis of the Golay code.

    This basis is computed from the definition of the Golay code as a 
    lexicographic code, see [SPLG], Ch. 11,  section 17.  
...
class HexacodeToGolay(object):
    """This class deals with the mapping from the Hexacode to the MOG
    
    This is described in [SPLG], ch. 11, section 5. Rows of the MOG are
    numbered from 0 to 5 (left to right). Columns of the MOG are numbered
    from 0 to 3 (top to bottom). MOG entries are represented as integers
    with bit 4*i+j (of valence 2**(4*i+j)) corresponding to the MOG entry
    in row i, column j:
    ...
    has 'score' j, see [SPLG], ch. 11, section 5.
    """
    ...
    def better_cocode_basis(cls, verbose=False):
        ...
        def e(i,a):
            ...
            v[i] = a
            return v
        hexa = lmap(e, [2,2,1,1,0,0], [3,2,3,2,3,2])
    ...
    @classmethod
    def basis(cls):
        """Return basis for the GF(2)**24 to be used in the Goay code
...
def make_syndrome_table(recip_basis):
     ...
     C1 = (24 << 5) | (24 << 10) 
     for i in range(24):
         b_i = recip_basis[i] & 0xfff
         syndrome_table[b_i >> 1] =  i | C1 
         for j in range(i+1,24):
             b_j = b_i ^ recip_basis[j]
             for k in range(j+1,24):
                 b_k = (b_j ^ recip_basis[k]) & 0xfff 
                 syndrome_table[b_k >> 1] = i | (j<<5) | (k<<10)  
...
def make_octad_tables(basis):
    ...
    Given an octad v in 'gcode' representation with 0 <= v < 0x1000, 
    the number of the corresponding octad is:

       (oct_enc_table[v >> 1] >> 1) + 3 * (v >> 4) - oct_enc_offset .

    A vector v in 'gcode' representation is an octad if the 
    following condition holds:

       (v ^ y) & 1 == 0  and y < 254 for y = oct_enc_table[v >> 1] . 

    """
    codewords = lin_table(basis)
    oct_dec_table = numpy.zeros(759, dtype = uint16)
    octad = 0
    d = {}
    for gcode, vector in enumerate(codewords[:4096]):
        if bw24(vector) == 8:
            oct_dec_table[octad] = gcode
            d[gcode] = octad - 3 * (gcode >> 4)
            octad += 1
    assert octad == 759
    ...
    for gcode, dict_value in d.items():
        ...
        oct_enc_table[gcode >> 1] = (gcode & 1) + 2 * new_value
...
class Mat24Tables(object):
    ...
    def vect_to_bit_list(v1):
        ...
        The function returns the pair (l1, l0).
        """
    ...
    @classmethod
    def gcode_to_octad(self, v1):
       v1 &= 0xfff
       y = int(self.oct_enc_table[v1 >> 1])
       if (v1 ^ y) & 1 or y >= 254:
           raise ValueError("Golay code vector is not an octad")
       return (y >> 1) + 3 * (v1 >> 4) - self.oct_enc_offset 
    ...
    def cocode_syndrome(cls, c1, u_tetrad=24):
        ...
        bad = (u_tetrad == 24) & (c1 + 1) & 1
        u_tetrad -= (u_tetrad + 8) >> 5  # change u_tetrad == 24 to 23
        y = - ((c1 + 1) & 1)             # y = 0 if c1 is odd else -1
        c1 ^= cls.recip_basis[u_tetrad] & y
        y  &=   1 << u_tetrad  
        syn = cls.syndrome_table[ (c1 >> 1) & 0x7ff ]
    ...
    @classmethod
    def syndrome(cls, v1, u_tetrad=0):
    ...
    def gcode_weight(cls, v1):
        ...
        """
        t = - (v1 & 1)
        w = (cls.theta_table[(v1 & 0xfff) >> 1] >> 12) & 7
        return ((w & 7) ^ t) + (t & 7)  
