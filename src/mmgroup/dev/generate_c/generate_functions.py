...
def format_item(n, format = None):
    ...
    At present the following integer formats are supported:
...
def prepend_blanks(string, length):
    """Prepend length blank characters to each line of string"""
...
class UserDirective(object):
    """Convert a function for use in a code genreator directive

    Each code genreator directive corresponds to a function. Evaluating
    ...
    The list of arguments of a code genreator directive is necessarily 
    ...
    function f, which is the first argument passed to the constructor.
    ...
    value in the dictionary 'directives' passed to the constructur of 
    class make_c_tables.TableGenerator.
    ...
    def __init__(self, f, eval_mode = "", prefix = False):
        ...
        'eval_mode' is a string specifying the syntax of the arguments
        in the string s containing the arguements. The i-th character of 
        the  string 'eval_mode' controls the evaluation of the i-th 
        parameter as follows:
          ...
          It 'eval_mode' is "." then a single parameter containing just 
          the given string is passed to function f as a single argument.


        'prefix' refers to an optional local variable of the code generator
...
class UserFormat(UserDirective):
    ...
    The code genenrator evaluates expressions in curly braces  {...}
    in the source code with the python .format() function.

    It is desirable that {int:2*3} evaluates to a string representing
    the value int(2*3) for some built-in and user-defined functions
    such as function int.

    Therefore we need a python object with name "int" that behaves 
    like the function int() when being called. That object should 
    also have a __format__ method that returns str(int(<expression>)) 
    ...
    replaces by any user-defiend function.

    Things get more interesing if we prepare a dictionary:
    
    tables = {"int": UserFormat(int),  "a": 3, "b":5}
    tables["int"].set_names(tables)             # done by the code generator
    "{int: a * b + int('1')}".format(**tables)  # evaluates to str(3*5+1)

    So UserFormat(f) creates an object the behaves like f when it is 
    called; and it returns str(f(arg1,arg2,...)) when it is invoked 
    in "{f:arg1,arg2,...}".format(f = f).

    Here the arguement list arg1,arg2,... is processed in acoording 
    to the same rules as the argument list of a user-defined directive
    in class UserDirective.

    The constructor of this class has parameters for processing the
    argument list (as in class UserDirective) and additional
    parameters for formatting the output of the underlying 
    function f   
    """
...
# Todo:  ast.Store is dangerout!!!
#        This should be legal in an ast.comprehension only!!!
...
def make_table(table, format=None):
    ...
    'table' may be anything iterable. Function format_item(x) is
    applied to each item x in the table.
