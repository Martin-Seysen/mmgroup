...
import mmgroup.dev.generate_c.generate_functions 


from mmgroup.dev.generate_c.generate_functions import isinteger
from mmgroup.dev.generate_c.generate_functions import format_item
from mmgroup.dev.generate_c.generate_functions import sizeof_format
from mmgroup.dev.generate_c.generate_functions import make_table
from mmgroup.dev.generate_c.generate_functions import py_doc_to_comment
from mmgroup.dev.generate_c.generate_functions import format_line
from mmgroup.dev.generate_c.generate_functions import line_as_comment
from mmgroup.dev.generate_c.generate_functions import named_table
from mmgroup.dev.generate_c.generate_functions import eval_codegen_args
from mmgroup.dev.generate_c.generate_functions import eval_codegen_table
from mmgroup.dev.generate_c.generate_functions import eval_codegen_for
from mmgroup.dev.generate_c.generate_functions import eval_codegen_else
from mmgroup.dev.generate_c.generate_functions import SaveDictFor
from mmgroup.dev.generate_c.generate_functions import safe_eval
from mmgroup.dev.generate_c.generate_functions import direct_single_arg
from mmgroup.dev.generate_c.generate_functions import prepend_blanks
from mmgroup.dev.generate_c.generate_functions import indent_subsequent_lines
from mmgroup.dev.generate_c.generate_functions import eval_codegen_join
...
from mmgroup.dev.generate_c.generate_functions import built_in_formats
...
class TableGenerator(object):
    ...
    For a more detailed description of these directives see file
    ...
    def __init__(self, tables, directives={}, verbose = False):
        ...
        #TODO: from here on the doc string has not been checked!!!

        'tables' is a dictionary of shape {name : table}.

           'name' is a string that is a valid python name.
           'table' is the python object to be referred by
           that name whenever an argument is evaluated as a
           python expression.
           

        'directives' is a dictionary of shape {function_name : function}
 
           The function is executed when " // %%function_name"
           occurs as keyword in a directive in the input file.
           The following arguments are evaluated and passed
           to the function. The function should return a string.
           This string is merged into the output .c file.

           See section 'Special cases for evaluating arguments in 
           user-defined directives' for more sophisticated 
           descriptions.
        """
...
"""
    ...
    def export_tables(self, file_name = None, append = False):
        """Export names of generated tables, deprecated!!!

        After calling self.generate() or self.iter_generate() you may
        call this function. It returns a dictionary Y containing the 
        names of the public tables generated by the last call to 
        self.generate() or self.iter_generate().

        Entries of the dictionary Y are of shape

           python_name_of_table : C_name_of_table

        When creating another C program with x.generate(), where x
        is an instance of class TableGenerator, you may call
        x.import_tables(Y) before calling x.generate().

        This makes the names of these exported tables available for 
        the code generation process in instance x of this class.
        More specifically, this updates the internal dictionary
        'names' used for formatting expressions in curly braces.

        The user is responsible for including the appropiate header
        files, there is no automatism for this task.

        If 'file_name' is set, a file with name 'file_name' is created.
        This file contains the dictionary Y as python code in the form:

        TABLE_NAMES = Y 

        If 'append' is set, that python code is appended to the file.
        """ 
        raise ValueError("Function no longer supported")
        names = self.exported_table_names
        if not file_name:
            return names
        with open(file_name, "at" if append else "wt") as f:
            if not append:
                print(
                  "# This file has been created automatically, Do not edit!\n", 
                  file = f
                )
            print(
                """\n# The following constant has been created by an instance of
# class make_c_tables.TableGenerator""", file = f
            )
            if self.source_name:
                print("# C skeleton source file name is:\n#    %s" %
                             self.source_name, file = f)
            names = ",\n    ".join(str(names).split(","))
            print("TABLE_NAMES = %s\n\n" % names, file = f) 


    def import_names(self, names):
        """Import names from the dictionary 'names'. deprecated!!!

        This updates the internal dictionary 'names', which is
        uses for formatting expressions in curly braces.
 
        This method must be invoked before a code generation process.
        Several such invocations are possible.

        This does not affect the dictionary 'tables' given in
        the constructor. You may pass names and constants in
        'names', but no data for tables to be generated.
        """
        raise ValueError("Function no longer supported")
        for _, entry in names.items():
            if isinstance(entry, UserFormat):
                entry.set_code_generator(self)
    ...
    def generate(self, source, c_filename, h_filename=None):
        """Generate .c file and .h file from source file

        Source file 'source' is processed according to the rules
        given in the description of this class. This generates a
        .c file with name 'c_filename' and a .h file with name
        'h_filename'. The extension must be part of 'c_filename'
        and 'h_filename'. If any of these two parameters is None,
        the corresponding output is dropped.
    ...
    def generate_pxd(self, pxd_file, h_file_name, source = None):
        """Create .pxd file 'pxd_file' from last call to generate().

        In the last call to method generate() all exported functions 
        ...
        Here 'h_file_name' is given as parameter and the exported
        functions listed below that statement are those collected by
        the last recent call to method generate().

        'pxd_file' is the name of the output file.

        An optional 'source' content is written into the output file
        ...
        print('cdef extern from "{0}":'.format(h_file_name), file = f)  
...
def make_doc(source_file, output_file, tables = None):
    """Extract documentation from C file generated by class TableGenerator

    If one of the TableGenerator directives EXPORT or COMMENT is found
    in the source file, everything of the source file is copied into the
    output file up to and inluding the first comment.
    ...
    'tables' is an optional dictionary that plays the same role 
    as in the constructor of class TableGenerator.

    'source_file may' be:
        - the name of a C source file
        - a string containing C statements with at least one '\n'
...
def c_snippet(source, *args, **kwds):
    ...
    Thus c_snippet(source, x, a = 'b') has pretty much the same effect 
    as source.format(x, a = 'b'). This means that positional and 
    keyword arguments of this function are eventually passed to to the 
    standard string formatting method .format() with the following 
    exceptions: 
    ...
    In order to achieve a similar effect as generating C code with
         ...
         tg.generate(source_file, c_file, _ )
