"""Abstact model of a representation the monster group
...
used for vector addition and scalar multiplicaton, and also for 
right multipliction of a vector by a group element. Optionally, a 
...
The monster group Mm acting on R_p by right multiplication is modeled
...
ajusted to a fast C implementation of the group operation on vectors.
...
vector v. Item assingement is done in the same way as in numpy. 
...
AbstractRepVector to its sparse representaion. This function 
should return a numpy array containg the sparse representaion.
...
class AbstractMmRepVector(AbstractRepVector):
    def __init__(self, space):
        self.space = space
        """This method should constuct a zero vector"""
    ...
    def as_sparse(self):
        ...
        The sparse reprsentation is returned as a one-dimensional
        numpy array of type numpy.uint32.

             starts at bit    bit length
        tag     25               3
        i0      14              11
        i1       8               6
        value    0               8

        This corresonds to the tuple (value, tag, i0, i1). 
    ...
    def as_tuples(self):
        ...
        The function returns a list of tuples  (value, tag, i0, i1).
        Here (tag, i0, i1) describes a basis vector and 'value'
        is the co-ordinate of that vector.
    ...
    def get_sparse(self, a_sparse):
        ...
        'a_sparse'. It updates the corrsponding values in 'a'
    ...
    def projection(self, *args):
        """Return projection of a vector onto a subspace

        Each argument describes a subspace of R_p generated by basis 
        vectors in the same was as in function sparse_subspace(). 
        Thus an argument may be e.g. an array-like object containing 
        indices of basis vectors in sparse representation. It may
        also be a tuple (tag, i0, i1) describing a basis vector. If
        an argument is a vector u in R_p, then it describes the 
        subspace spanned by all basis vectors where the coordinate 
        of u is not zero. Let V be the span of all these subspaces.

        The function returns the projection v of the vector 'self'  
        to the subspace V. The coordinates of v corresponding to   
        basis vectors not in V are set to zero.
        """
...
class AbstractMmRepSpace(AbstractRepSpace):
    ...
    number. A scalar multiple of a basis vector may be witten as 
    ...
    vector or one of following letters for generaring a random 
    ...
    - v may be an element of another space V.
      If there is a hmomorphism phi from V to this space 
    ...
    - v may be the integer 0. This means the the zero vector.
    """
    ...
    def setitems_sparse(self, v, a_sparse):
        ...
        assigning differnet values to the same co-ordinate.
    ...
    def from_sparse(self, a_sparse):
        """Construct a vector from an array containing its sparse rep.

        Here a_sparse is an array-like list of unsigned 32-bit integers 
        that contains the sparse representation of a vector as described  
        in the documentation of the module  mm_aux.c. 
    ...
    def vector_get_item(self, v1, item):
        ...
        if not isinstance(item, tuple):
            item = (item,) 
        shape, a_sparse = sparse_from_indices(*item)
...
def sparse_subspace(*args):
    ...
    - A set or an array-like object containing integers
        ...
        of basis vectors, igoring the scalar factor. Integers not
