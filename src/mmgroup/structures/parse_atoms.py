...
hex index         =  hex_number | (digit, {hex digit}, ("h" | "H")) ;
...
atoms and idetnifiers as python identifers. 
...
class AtomDict(object):
    ...
    def __init__(self, reducer, identifiers = {}):
        self.identifiers = identifiers
        self.reducer = reducer

    def __getitem__(self, s):
        if self.match_atom.fullmatch(s):
            indices = map(self.parse_index, s[2:].split("_"))
...
def ihex(x):
   s = "%xh" % x
   return s if s[:1].isdigit() else "0" + s
