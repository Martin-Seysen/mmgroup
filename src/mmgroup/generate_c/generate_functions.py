...
class UserFormat(UserDirective):
    ...
    Here the argument list ``arg1, arg2,...`` is processed in acoording 
    ...
    :param f:
        ...
        A callable function which is executed when the code generator
        parses a user-defined formatting string in curly barces ``{...}``. 
    ...
    :param fmt:
        ...
        if ``fmt`` is a callable funtion  then ``str(f(*args))`` is 
...
def make_table(table, format=None):
    ...
    Parmeter ``table`` may be anything iterable. Function 
