r"""The C code generator generates code from a source file automatically.
    ...
    There is a limited number of built-in python functions (specifed 
     ...
     * ``EXPORT``   (arguments see below)
        ...
        creating prototypes.
        The ``EXPORT`` directive may preceed a line with a ``USE_TABLE`` 
        directive.
     ...
     * ``EXPORT_TABLE`` (no arguments)
        ...
        There are, however sublte differences, see directive 
        ``EXPORT_KWD``.
     ...
     * ``USE_TABLE``   (no arguments)
        ...
        passed to the constructur of class ``TableGenerator``. The
        ...
        or via string formating with curly braces.  
