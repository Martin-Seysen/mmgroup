...
@pytest.mark.space
def test_sparse_space():
    mmv7 = SparseMmSpace(7)

    v = mmv7.unit('X', 0x6d3, 23)
    w =  mmv7.unit('Y', 0x126, 20)
    v += mmv7.unit('X', 0x126, 20) / 4
    maxlen = 0
    print( -17 * v / 8 + (w >> 16))
    for i, (v, g1, g2) in enumerate(mmv_testdata(mmv7)):
        al = (v * g1) * g2
        ar = v * (g1 * g2)
