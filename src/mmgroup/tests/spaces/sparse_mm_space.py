...
def mul_Tp(tag, octad, sub, g):
    ...
    s = d1 >> 12
    s ^= eps & 1 & m24.suboctad_weight(sub) 
...
def mul_XYZt(index, tag, d, i, g):
    dd = d << 1
    e = g.exp % 3
    return  XYZ_t_SIGNS[index][e](dd,i), XYZ_t_TAGS[index + e], d, i
...
class NonMonomialOp_l:
    ...
    MSYM16 = pm_mat_from_function(lambda i, j: w2_gray((i ^ j) << 1), 16)
    MDIAG16 = -pm_diag_from_function(lambda i: w2_gray(i << 1), 16)
    ...
    def imul_group_atom(self, v, g, w):
        ...
        for vu, v_coord in v.data.items():
            ...
            try:
                ...
                wu = (wtag << 25) + (wi << 14) + (wj << 8)
                assert 0 <= v_coord <= p
                if (sign & 1):  v_coord = p - v_coord
                w.data[wu] = v_coord
        if g.tag in "tl":
