...
them. There are applications of axes where one has to generate axes
with utmost speed. E.g. for computing normalizers of small subgroup
of the Monster one has to generate a large number of axes and do
computionas with them, see :cite:`BPW16` for background.

The class ``RandomAxis`` implemented here allows very fast generation
of a random axis and of the corresponding 2A involution.

Another application of the functions in this module is a fast
...
class RandomAxis:
    ...
    def _get_vectors(cls):
        cls.indices, cls.data, cls.prob =  get_axis_reps(axis_class)
    ...
    def g(self, group = MM):
        ...
        internal representaion of an element of the Monster is returned
        as a numpy array of unsigned 32-bit integers.
        """
...
class RandomBabyAxis(RandomAxis):
    ...
    Details will be documented in a future version.
    """
    axis_class = BabyAxis
    MAX_N_G_x0 = MAX_NUM_H
    v15_start_name = V_AXIS_OPP 
...
def rand_mm_element(group_name = "M", group = MM):
    """Fast generation of a uniform random element of the Monster

    The function returns a uniform distributed random elements.
    ...
    if group_name == "G_x0":
        ...
        mm = rand_G_x0_baby()[0]
        if group_name != "H":
            mm = np.concat(mm, RandomBabyAxis().g('a'))
        elif group_name != "B":
            mm = np.concat(mm, RandomAxis().g('a'))
        elif group_name != "M":
            ERR = "Illegal name %s for subgroup of Monster"
            raise ValueError(ERR % groupname)
    if group == 'a':
        return mm
    return group(mm)
