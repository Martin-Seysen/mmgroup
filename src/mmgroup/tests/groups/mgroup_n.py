...
# K1:  y_(-1) = x_(-Omega)
dict_kernel0_yx = {0:0, 0x1000:0x1001, 1:0x1000, 0x1001:1, }
dict_kernel1_yx = {0:0, 0x1000:0x1001}

dict_y = {
    None: (0, {0:0}),
    1: (0x1000, dict_kernel1_yx),
    0: (0x1001, dict_kernel0_yx),
}
...
def kernel_rule_yx(group, word):
    global n_rules
    n_rules += 1
    y, x = word[0].pl,  word[1].pl & 0x1fff
...
def rule_pt(group, word):
    ...
    n_rules += 1
    p, t = word[0], word[1]
    exp = 3 - t.exp  if p.cocode & 1 else t.exp
    t1 = tl_Atom('t', exp % 3)
    return [t1, p]
